defmodule LinkexWeb.HomeController do
  use LinkexWeb, :controller

  def index(conn, _params) do
    conn
    |> assign(:url_host, Linkex.Url.conn_host(conn))
    |> render("index.html")
  end

  def urls(conn, _params) do
    all = 
      Linkex.Data.Link.all()
      |> Enum.map(&(elem(&1, 1)))
  ...
  def key_direct(conn, params) do
    key = params["key"]
    url = Linkex.Data.Link.lookup(key)

    if url != nil do
      Linkex.Data.Log.record(url)
      conn
